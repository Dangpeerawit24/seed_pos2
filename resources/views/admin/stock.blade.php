    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white shadow-md rounded-lg p-4 flex flex-col justify-between">
            ...
            <div class="mt-4 flex justify-between">
                ...
                <a href="/admin/stock/movements/{{ $product->id }}" class="text-blue-500 text-sm underline hover:text-blue-700">รายละเอียด</a>
            </div>
        </div>
        @endforeach
    </div>
...
function submitStockForm() {
    ...
    if (operation === 'add') {
        form.action = "{{ route('stock.add', ':id') }}".replace(':id', productId);
    } else {
        form.action = "{{ route('stock.reduce', ':id') }}".replace(':id', productId);
    }
