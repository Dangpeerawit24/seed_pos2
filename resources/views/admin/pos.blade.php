...
@Section('content')
    <div class="row flex flex-col lg:flex-row gap-1 h-full fixed overflow-hidden scrollbar-hide">
                ...
                @foreach ($products as $product)
                    <div class="product-card bg-sky-100 w-auto md:w-auto h-auto flex items-end  m-1 p-2 rounded-lg shadow-xl border-2 border-blue-300 lg:hover:scale-105 transition-transform duration-500 ease-in-out	"
                        ...
                        <a id="pos" href="#"
                            ...
                            <p class=" text-md font-bold text-black">คงเหลือ {{ $product->stock_quantity }}</p>
        ...
        <div class=" hidden w-0 lg:flex lg:w-2/5 lg:p-5 bg-white overflow-hidden rounded-lg shadow-lg overflow-y-auto">
            <div class="w-full">
                <div>
                    <h1 class="text-xl mb-2">ลูกค้า</h1>
                    <!-- Dropdown เลือกสมาชิก -->
                    <select id="memberSelect" class="border p-2 rounded w-full" onchange="onMemberChangeDesktop()">
                    ...
                    <!-- หรือ พิมพ์ชื่อเอง -->
                    <label for="customCustomerName" class="block mt-2">หรือใส่ชื่อ (ไม่ใช่สมาชิก)</label>
                    <input type="text" id="customCustomerName" class="border p-2 rounded w-full"
                        placeholder="ชื่อผู้ซื้อ" oninput="onNameChangeDesktop()" />
                </div>
                ...
                <!-- ปุ่มชำระเงิน -->
                <div>
                    <button id="checkoutButton" class="px-4 py-2 h-16 w-full bg-sky-400 text-white rounded hover:bg-sky-600"
                        onclick="checkout()">
                ...
                <!-- รายการในตะกร้า -->
                <div class="overflow-y-auto h-2/5 px-4" id="cartContainer">
                    <!-- รายการสินค้า -->
                    <!-- สินค้าจะแสดงที่นี่ด้วย JavaScript -->
                </div>
            </div>
        </div>
        <div class=" w-full h-auto p-2 flex flex-col fixed bottom-0 lg:hidden z-50 bg-white rounded-t-xl">
            <!-- เปลี่ยน h-32 เป็น h-auto เพื่อให้รองรับการใส่ dropdown -->
            <div class="flex flex-col w-full p-2">
                <!-- ส่วนเลือกสมาชิก -->
                <!-- ปุ่ม + ส่วนที่จะย่อ/ขยาย -->
                <div class="mb-2 px-4 flex items-center justify-between">
                    <label class="block text-lg font-medium text-gray-600">เลือกลูกค้า</label>
                    <!-- ปุ่ม caret -->
                    <button type="button" onclick="toggleMemberSectionMobile()" class="text-2xl text-gray-600 font-bold">
                        ^
                    </button>
                </div>

                <!-- เพิ่ม hidden ไว้ตั้งแต่แรก -->
                <div id="memberSectionMobile" class="hidden transition-all duration-300">
                    <div class="mb-2">
                        <select id="memberSelectMobile" class="border p-2 rounded w-full" onchange="onMemberChangeMobile()">
                            <option value="">--- เลือกสมาชิก ---</option>
                            @foreach ($members as $member)
                                <option value="{{ $member->id }}">{{ $member->name }}</option>
                            @endforeach
                    ...
                    <div class="mb-2">
                        <label for="customCustomerNameMobile" class="block text-md">หรือใส่ชื่อ (ไม่ใช่สมาชิก)</label>
                        <input type="text" id="customCustomerNameMobile" class="border p-2 rounded w-full"
                            placeholder="ชื่อผู้ซื้อ" oninput="onNameChangeMobile()" />
                    </div>
                </div>
    ...
    <div id="paymentModal" class="fixed inset-0  px-2 flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-lg ">
            ...
            <div class="flex justify-around h-14 p-0 m-0 bg-gray-100">
                ...
                <button class="font-bold w-1/2 h-14 " onclick="switchTab('online')" id="onlinebtn">ธนาคาร</button>
                <button class="font-bold w-1/2 h-14 " onclick="switchTab('borrow')" id="borrowbtn">ยืมสินค้า</button>
            </div>
            ...
            <div class="p-4 h-[350px]">
                ...
                <div id="borrowTab" class="h-[350px] hidden flex flex-col tab-content items-center text-center">
                    <div class=" w-full px-16 text-center">
                        <div class="flex justify-between w-full">
    ...
    <script>
        function toggleMemberSectionMobile() {
            const section = document.getElementById('memberSectionMobile');
            // เช็คว่ามี hidden อยู่ไหม
            if (section.classList.contains('hidden')) {
                section.classList.remove('hidden'); // ถ้ามี => เอาออก (แสดง)
            } else {
                section.classList.add('hidden'); // ถ้าไม่มี => ใส่ (ซ่อน)
            }
        }
    </script>

    <script>
        function onMemberChangeDesktop() {
            const deskSelect = document.getElementById('memberSelect');
            const mobSelect = document.getElementById('memberSelectMobile');
            const deskName = document.getElementById('customCustomerName');
            const mobName = document.getElementById('customCustomerNameMobile');

            // อัปเดตค่ามือถือให้ตรงกับ desktop
            mobSelect.value = deskSelect.value;

            // ถ้าเลือกสมาชิกแล้ว => เคลียร์ช่องชื่อ
            if (deskSelect.value) {
                deskName.value = '';
                mobName.value = '';
            }
        ...
        function onMemberChangeMobile() {
            const deskSelect = document.getElementById('memberSelect');
            const mobSelect = document.getElementById('memberSelectMobile');
            const deskName = document.getElementById('customCustomerName');
            const mobName = document.getElementById('customCustomerNameMobile');

            // อัปเดตค่าของ desktop ให้ตรงกับ mobile
            deskSelect.value = mobSelect.value;
        ...
        function onNameChangeDesktop() {
            ...
            // ดึงชื่อ desktop
            mobName.value = deskName.value;
            // ถ้าพิมพ์ชื่อ => เคลียร์ select
            if (deskName.value) {
                deskSelect.value = '';
                mobSelect.value = '';
            }
    ...
    <script>
        ...
        function switchTab(tab) {
            ...
            if (tab === 'cash') {
                ...
                document.getElementById("borrowbtn").classList.add("bg-gray-200", "text-gray-800");
                document.getElementById("borrowbtn").classList.remove("bg-sky-400", "text-white");
            } else if (tab === 'online') {
                ...
                document.getElementById("borrowbtn").classList.add("bg-sky-400", "text-white");
                document.getElementById("borrowbtn").classList.remove("bg-gray-200", "text-gray-800");
    ...
    <!-- Modal แก้ไขราคา -->
    <div id="editPriceModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-2xl font-bold">แก้ไขราคาต่อหน่วย</h2>
                <button onclick="closePriceModal()" class="text-gray-600 hover:text-gray-800">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" id="priceInput"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-sky-400" placeholder="0.00" />
            </div>

            <button onclick="savePrice()" class="w-full py-3 bg-blue-500 text-white text-2xl font-bold rounded-lg">
                ตกลง
            </button>
        </div>
    </div>
    <script>
        let currentPriceProductId = null; // เก็บ ID สินค้าที่กำลังแก้ไขราคา

        function openPriceModal(productId, productName, currentPrice) {
            currentPriceProductId = productId;

            // ใส่ราคาปัจจุบันลงใน input
        ...
        // ฟังก์ชันบันทึกราคา
        function savePrice() {
            if (!currentPriceProductId || !cart[currentPriceProductId]) {
                closePriceModal();
                return;
            }
            ...
            // อัปเดตราคาใน cart
            cart[currentPriceProductId].price = newPrice;

            // อัปเดตตะกร้าและปิด modal
            updateCart();
            closePriceModal();
        }
    </script>
    ...
    <script>
                    ...
                    <div class="w-full mt-2 p-5 px-4 bg-sky-50 rounded-xl overflow-y-auto">
                        <div class="flex justify-between">
                            ...
                            <div class=" flex items-center">
                                ...
                                <!-- ปุ่มแก้ไขราคา -->
                                <button class="px-4 py-2  ml-4 bg-white rounded hover:bg-yellow-50"
                                    onclick="openPriceModal(${productId}, '${item.name}', ${item.price})">
        ...
        async function submitPayment() {
            ...
            // ดึงค่า member/customer
            const memberId = document.getElementById('memberSelect').value;
            const customName = document.getElementById('customCustomerName').value.trim();

            // สร้างตัวแปรที่จะส่งในทุกกรณี
            const membership_id = memberId ? parseInt(memberId) : null;
            const customer_name = customName || null;
            ...
            if (paymentMethod === 'cash') {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        const changeAmount = data.order.change || 0;
                        let title;
                        if (data.order.payment_method === 'borrow') {
                            title = 'ใบยืมสินค้า';
                        } else {
                        ...
                        Swal.fire({
                            ...
                            window.open(`/orders/${orderId}/print?title=${encodeURIComponent(title)}`, '_blank',
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        Swal.fire({
                            ...
                            window.open(`/orders/${orderId}/print?title=ใบเสร็จรับเงิน`, '_blank',
                                'width=600,height=800');
                            closePaymentModal();
                            cart = {};
                            updateCart();
            ...
            } else if (paymentMethod === 'borrow') {

                if (!membership_id) {
                    Swal.fire({
                        icon: 'error',
                        title: 'กรุณาเลือกสมาชิก',
                        text: 'กรุณาเลือกสมาชิก เนื่องจากคุณเลือกหัวข้อการยืมสินค้า',
                        confirmButtonText: 'ตกลง',
                    });
                    return;
                }
                ...
                const formData = new FormData();
                formData.append('payment_method', 'borrow');
                formData.append('total_amount', totalAmount);
                formData.append('membership_id', membership_id);
                formData.append('customer_name', customer_name);
                formData.append('cart', JSON.stringify(cartItems));
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        Swal.fire({
                            ...
                            window.open(`/orders/${orderId}/print?title=ใบยืมสินค้า`, '_blank',
                                'width=600,height=800');
                            closePaymentModal();
                            cart = {};
                            updateCart();
